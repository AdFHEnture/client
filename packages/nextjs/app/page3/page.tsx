...
const Page3Page = () => {
    ...
    const [selectedCheckboxesUser, setSelectedCheckboxesUser] = useState<string[]>([]);
    ...
    const handleCheckboxChangeUser = (option: CheckboxOption) => {
        if (selectedCheckboxesUser.includes(option.value)) {
            setSelectedCheckboxesUser(selectedCheckboxesUser.filter((value) => value !== option.value));
        } else {
            if (selectedCheckboxesUser.length < 5) {
                setSelectedCheckboxesUser([...selectedCheckboxesUser, option.value]);
            } else {
                alert("You can select a maximum of 5 options.");
            }
        }
    ...
    return (
        <div className="flex flex-col items-center h-screen gap-4 p-12">
            <div className="flex flex-col items-center justify-center">
                <h1 className="text-6xl font-bold bg-gradient-to-r from-white to-[#E7F3C6] bg-clip-text text-transparent">AdFHEnture</h1>
                <h2 className="text-xl text-white text-center">Neque porro quisquam est qui <br /> dolorem</h2>
            </div>
            <div>
                <button onClick={() => setUserType('advertiser')} className={`w-[250px] h-[50px] rounded-l-lg ${userType === 'user' ? "text-white text-xl bg-[#262626]" : "text-black text-xl bg-gradient-to-r from-white to-[#E7F3C6]"} `}>
                    For Advertisers
                </button>
                <button onClick={() => setUserType('user')} className={`w-[250px] h-[50px] rounded-r-lg ${userType === 'advertiser' ? "text-white text-xl bg-[#262626]" : "text-black text-xl bg-gradient-to-r from-white to-[#E7F3C6]"} `}>
                    For Users
                </button>

            </div>
            {userType === 'advertiser' ? <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-3/5 h-full justify-center items-center">
                    ...
                    <div className="flex flex-col justify-center  w-1/2 mx-2 h-3/4 rounded p-4 relative">
                        ...
                        <div className="flex flex-col w-full">
                            ...
                            <Slider
                                aria-label="Date"
                                defaultValue={1}
                                onChange={(e: { target: { value: SetStateAction<number>; }; }) => { setWeek(e.target.value) }}
                    ...
                    <div className="flex flex-col justify-center w-1/2 h-full mx-2 rounded p-2 relative">
                        ...
                        <div className="grid grid-cols-2">
                            {checkboxOptions.map((option, index) => (
                                <div
                                    key={index}
                                    className="flex justify-center items-center rounded-lg p-2 my-2 cursor-pointer"
                                >
                                    <input
                                        type="checkbox"
                                        id={`checkbox-${index}`}
                                        ...
                                        onChange={(e) => { e.stopPropagation(); handleCheckboxChangeAdvertiser(option); }}
                ...
                <button type="submit" className="w-[250px] h-[50px] text-black text-xl bg-gradient-to-r from-white to-[#E7F3C6]">
                    Pay & Submit
                </button>
            </form> :
                <div className="flex flex-col justify-center items-center w-full h-full mx-2 rounded p-4 relative">
                        ...
                        {checkboxOptions.map((option, index) => (
                            ...
                            >
                                <input
                                    ...
                                    name={option.name}
                                    value={option.value}
                                    checked={selectedCheckboxesUser.includes(option.value)}
                                    onChange={(e) => { e.stopPropagation(); handleCheckboxChangeUser(option); }}
                                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-[#E7F3C6]"
                                />
                    ...
                    <button type="submit" className="w-[250px] h-[50px] text-black text-xl bg-gradient-to-r from-white to-[#E7F3C6] mt-8">
                        Submit
                    </button>
                </div>
            }
